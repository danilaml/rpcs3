...
namespace
{
	...
	u32 to_gl_internal_type(rsx::vertex_base_type type, u8 size)
	{
		...
		if (type > rsx::vertex_base_type::ub256)
			throw std::runtime_error(fmt::format("OpenGL error: unknown vertex base type 0x%X. file '%s' function '%s' line %ld", (u32)type, __FILE__, __FUNCTION__, __LINE__));
...
void GLGSRender::end()
{
	...
	if (draw_command == rsx::draw_command::inlined_array)
	{
		...
		for (int index = 0; index < rsx::limits::vertex_count; ++index)
		{
			...
			texture->copy_from(buffer->id(), gl_type);
